...
if __name__ == '__main__':
    ...
    try:
        while True:
            ...
            for substring8 in devices:
                while(Lo<=2):
                    print ("FLMS04868")
                    acquired = request_spectrum(sp.usb_device, sp.packet_size, sp.spectra_ep_in, sp.cmd_ep_out)
                    samplespec = (np.linspace(1,len(acquired),num=len(acquired),dtype="int"))
                    np.savetxt(time.strftime("%Y%m%d_%H%M%S")+"_FLMS04868"+".csv", np.vstack((samplespec,acquired)).T, delimiter=', ')
                    print(f"(acquired) : {acquired}")
                    plt.title("FLMS04868_"+time.strftime("%Y%m%d_%H%M%S"))
                    plt.plot(acquired)
                    plt.pause(5)
                    plt.savefig(time.strftime("%Y%m%d_%H%M%S")+"_FLMS04868"+".png",format="png",dpi=Dots1Inch_height)
                    plt.cla()
                    print("exiting and cleaning up USB comms ...")
                    drop_spectrometer(sp.usb_device)
