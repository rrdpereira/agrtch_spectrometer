        ...
        if sp.usb_device is None:
            ...
            try:
                while True:
                    ...
                    np.savetxt(time.strftime("%Y%m%d_%H%M%S")+"_001"+".csv", np.vstack((samplespec,acquired)).T, delimiter=', ')
                    print(f"(acquired) : {acquired}")
                    plt.title("001_"+time.strftime("%Y%m%d_%H%M%S"))
                    plt.plot(acquired)
                    plt.pause(5)
                    plt.savefig(time.strftime("%Y%m%d_%H%M%S")+"_001"+".png",format="png",dpi=Dots1Inch_height)
                    plt.cla()
